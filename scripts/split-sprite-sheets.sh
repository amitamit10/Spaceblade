...
split_player_action_frames() {
  ...
  for source_x in "$@"; do
    local y=$((row * 96))
    convert "$source" -crop "96x96+${source_x}+$y" +repage \
      "$output_dir/$name-$(printf '%02d' "$frame_index").png"
    frame_index=$((frame_index + 1))
  done
}

split_sheet player 96 96 "walk:1:6"
# These are source windows around the real populated poses in the supplied sheet.
split_player_action_frames slash 2 0 96 192 384
split_player_action_frames charge 3 0 32 192
split_player_action_frames heavy 4 0 96 192 288 384 480
split_player_action_frames dodge 5 0 96 192
split_player_action_frames parry 6 0 96
split_player_action_frames hurt 7 0 96
split_player_action_frames dead 8 0 96
