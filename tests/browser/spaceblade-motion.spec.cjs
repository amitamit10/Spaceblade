...
async function startPhaserGameplay(page) {
  ...
  await expect(page.locator("canvas")).toHaveAttribute("data-spaceblade-building", "interior");
  await expect(page.locator("canvas")).toHaveAttribute("data-spaceblade-obstacles", "6");
...
test("standalone player and enemy frames advance during gameplay", async ({ page }) => {
  ...
  expect(first.player).toContain("/assets/public/opengameart-space-soldier/");
